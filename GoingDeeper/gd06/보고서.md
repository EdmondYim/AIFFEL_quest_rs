...
가장 많은 오류가 삭제/삽입(I/E/T/L/R/... -> <del>, <ins> -> I/R/A/L/...)입니다. 

얇은 획·세로줄을 놓치거나 노이즈로 잘못 읽는 경향이 큽니다. 

치환은 A<->O/E, I->L, E->O처럼 원형/세로획끼리 헷갈립니다.
...
길이별 정확도: 10자 이후부터 급락(10자 86.8% → 14자 66% → 16자 46%). 긴 시퀀스에서 CTC alignment가 불안정하거나 시퀀스 길이(35 timestep) 대비 정보가 부족한 것 같습니다. 긴 단어 대상 보완 필요.

**길이에 따른 한 문자열을 다른 문자열로 바꾸기 위해 필요한 최소한의 연산 횟수(Top Counts)**
...
편집거리 분포: 대부분 ED=0인 깨끗한 샘플이지만, ED=1이 많은 구간(8~11자)에서 부분 오류가 주로 발생합니다. 한 글자 삑사리 방지가 핵심 개선 포인트입니다.

### 개선방안

가장 큰 오류 패턴은 삭제/삽입: I/E/T/L/R/... -> <del>과 <ins> -> I/R/A/L/...이 상위권입니다. 

두께/세로획/인접 노이즈에 약한 듯하니, 학습/추론 양쪽에서 세로줄/점 노이즈, 얇은 획을 키우는 증강(erosion/dilation, gaussian blur, contrast jitter, random vertical line noise)을 추가

치환도 A<->O/E, I->L처럼 원형·세로획 혼동이 많습니다. 동일하게 대비/두께 증강, 그리고 0/1/O/I/L 류 샘플을 추가 보강

길이별 정확도는 10자 이후 급락(10자 86.8% → 12자 80.5% → 15자 58% → 17자 40%). 긴 시퀀스에서 CTC alignment가 불안정해 보입니다.
